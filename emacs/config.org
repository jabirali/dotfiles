...
** Python
TODO: We should get this to work also with Tramp. Currently, it tries to use IPython remotely if it finds it locally. In practice, that means setting a buffer-local variable in a python-mode hook.
#+begin_src emacs-lisp
  (use-package python
    :config
