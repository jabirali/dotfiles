...
- CDLaTeX :: Most snippet solutions are not as ergonomic as this. The VimTeX one comes close, but doesn't work in e.g. MarkDown contexts last I checked, which is one main place I do want it to work.
...
The package Comint-mime lets us show Matplotlib plots directly in the Inferior Python buffer. Similarly to VSCode's "Interactive Python".
#+begin_src emacs-lisp
