...
* Installation
** Mac port
The "Mac port" integrates best with MacOS, including full Retina (hiDPI) support and better trackpad integration. But if you use Emacs in a terminal, the NS port seems to work better.
...
** Ubuntu
My main server is currently running Ubuntu 24.04 LTS, which has Emacs 29.3 in the main repos.
#+begin_src sh
  sudo apt install emacs
#+end_src
Note that there are several versions available (=emacs-pgtk=, =emacs-nox=, =emacs-lucid=, etc.). As long as I'm using it remotely over SSH, it doesn't really matter which version is installed.
