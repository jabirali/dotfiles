...
** Operating system
Install the relevant Emacs version. The cask version unfortunately misses some features (most notably native compilation), but it has way fewer bugs related to e.g. running it from a terminal.
#+begin_src bash
  brew tap "railwaycat/emacsmacport"
  brew install --cask emacs-mac
#+end_src bash
    
Some tweaks to make the app look a bit nicer with =tab-bar-mode=.
#+begin_src bash
