...
** Package management
Since Emacs 29, =use-package= has been included in the base distribution. There are however some defaults we need to fix, notably third-party package repos and automatic installation.
...
By default, =use-package= uses the built-in =package.el= for package management, which can only install packages from Emacs repositories. The following integrates it with =quelpa= as well, thus permitting the direct installation of Emacs packages hosted on e.g. GitHub.
#+begin_src emacs-lisp
...
Code folding with =outline-minor-mode=. This is the package that =org-mode= is built on, and it integrates well with =evil-mode=.
#+begin_src emacs-lisp
  (use-package outline
    :custom
