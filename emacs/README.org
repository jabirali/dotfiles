...
* Emacs
This is my configuration for Emacs 29+ on macOS. My setup is mostly based around =org-mode= and =evil-mode=. (I use Emacs for Org-mode, but often use Vim for programming.)

** Package management
Bootstrap the Emacs package manager and enable the community package repositories.
...
This config is based around =use-package=. Since Emacs v29, this has been upstreamed into Emacs and no longer needs to be manually installed. From v30, it will also obtain a =:vc= keyword that allows package installation directly from Git repositories.
#+begin_src emacs-lisp
...
By default, Emacs tend to throw files all over the place. The =no-littering= package helps relocate various cache files, backup files, and autosave files into a single location.
#+begin_src emacs-lisp
